...
estimators = utils.randomizedsearch_CV_m(df, [xgb_reg, xgb_reg, xgb_reg], cols_to_be_dropped,
                                         one_hot_cat_vars, smooth_cat_vars, decrease_cat_vars,
                                         [params, params, params], weights=weights, trials=100)
...
utils.drop_columns(df_min_in, cols_to_be_dropped +
                   ['max_price'], variable_lists)
...
df_comp_min = utils.get_predictions(
    df_min_in, estimator, target, 'id', 'min_price_pred')
...
utils.drop_columns(df_max_in, cols_to_be_dropped +
                   ['min_price'], variable_lists)
...
df_comp_max = utils.get_predictions(
    df_max_in, estimator, target, 'id', 'max_price_pred')
...
utils.drop_columns(df_dif_in, cols_to_be_dropped +
                   ['min_price', 'max_price'], variable_lists)
...
tot_mae2 = mean_absolute_error(
    df_fin2['TRUE_MAX'], df_fin2['MAX']) + mean_absolute_error(df_fin2['TRUE_MIN'], df_fin2['MIN'])

df_fin3 = df_max.merge(df_dif, on="id").merge(df_min, on="id")
df_fin3['MIN'] = df_fin3['MAX'] - df_fin3['DIF']
tot_mae3 = mean_absolute_error(
    df_fin3['TRUE_MAX'], df_fin3['MAX']) + mean_absolute_error(df_fin3['TRUE_MIN'], df_fin3['MIN'])

df_out = df_min.merge(df_max, on="id").set_index("id")
df_out = df_out.merge(df_dif, on="id").set_index("id")
df_out['tmp'] = df_out['MAX']*weights[0] + \
    (df_out['MIN'] + abs(df_out['DIF']))*weights[1]
df_out['MIN'] = df_out['MIN']*weights[0] + \
    (df_out['MAX'] - abs(df_out['DIF']))*weights[1]
df_out['MAX'] = df_out['tmp']
df_out.loc[df_out['MAX'] < df_out['MIN'], ['MIN', 'MAX']
           ] = df_out.loc[df_out['MAX'] < df_out['MIN'], ['MIN', 'MAX']].mean(axis=1)
tot_mae = mean_absolute_error(
    df_out['TRUE_MAX'], df_out['MAX']) + mean_absolute_error(df_out['TRUE_MIN'], df_out['MIN'])
