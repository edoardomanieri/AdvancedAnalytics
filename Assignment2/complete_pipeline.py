...
_, mae = utils.full_CV_pipeline_m(df, estimators, cols_to_be_dropped,
                                  one_hot_cat_vars, smooth_cat_vars, decrease_cat_vars, weights=weights)
...
df_min = utils.fit_predict(df, estimator, target,
                           'id', 'MIN', report_file=report_file)
df_comp_min = utils.get_predictions(
    df, estimator, target, 'id', 'min_price_pred')
report_file.write("\n\n\n")
...
train_max = train_max.drop(train_max[~train_max['detachable_keyboard'].isin([
...
df_max = utils.fit_predict(df, estimator, target,
                           'id', 'MAX', report_file=report_file)
df_comp_max = utils.get_predictions(
    df, estimator, target, 'id', 'max_price_pred')
report_file.write("\n\n\n")
...
train_dif = train_dif.drop(train_dif[~train_dif['detachable_keyboard'].isin([
                           0, 1])].index).reset_index(drop=True)
...
df_dif = utils.fit_predict(df, estimator, target,
                           'id', 'DIF', report_file=report_file)
report_file.write("\n\n\n")
...
report_file.write(
    "replace values where max smaller than min with mean of two values \n")
report_file.write(
    "prediction is average of price predictions and predictions with difference \n")
report_file.close()
